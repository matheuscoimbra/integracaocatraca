package br.com.hsd.catraca.api.tcpcom;
...
public class TcpClient {
    ...
    public boolean connect(String host, int port, int timeout) {
        try {
            this.host = host;
            this.port = port;
            log.debug("Conectando ao ip " + host + ":" + port);
            this.socket = new Socket();
            this.socket = SocketFactory.getDefault().createSocket(this.host, 3000);

            this.cache = new Cache();
            this.os = new DataOutputStream(this.socket.getOutputStream());
            this.is = new DataInputStream(this.socket.getInputStream());
            this.receive = true;
            log.debug("Iniciando thread de conexão");
            this.startRead();
            this.connected = true;
            return true;
            ...
            public void run() {
                while(true) {
                    try {
                        ...
                        TcpClient.log.error(String.valueOf(var4));
                    }
